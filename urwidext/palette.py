# -*- coding: utf-8 -*-
from urwid import AttrSpec, Widget, Text, AttrMap
...
## Uses default theme from base16
## https://github.com/chriskempson/base16

# structure: color index, #hex, labels
colors16 = [
	### basic colors ###
	(0,  '#151515', ('00', 'black','b','default_bg') ),
	(8,  '#505050', ('03', 'dark gray','g3') ),
	(7,  '#d0d0d0', ('05', 'light gray','g5') ),
	(15, '#f5f5f5', ('07', 'white','w','default_fg') ),
	(1,  '#ac4142', ('08', 'red','r') ),
	(9,  '#ac4142', ('08', 'light red',) ),
	(11, '#f4bf75', ('0A', 'yellow','y') ),
	(2,  '#90a959', ('0B', 'green','gr') ),
	(10, '#90a959', ('0B', 'light green') ),
	(6,  '#75b5aa', ('0C', 'cyan','cy') ),
	(14, '#75b5aa', ('0C', 'light cyan',) ),
	(4,  '#6a9fb5', ('0D', 'blue','bl') ),
	(12, '#6a9fb5', ('0D', 'light blue') ),
	(5,  '#aa759f', ('0E', 'magenta','ma') ),
	(13, '#aa759f', ('0E', 'light magenta') ),
	(3,  '#8f5536', ('0F', 'brown','br') ),
	]
### extra colors from base16 ###
extra16 = [
	(10, '#a6e22e', ('light green', 'lgr', 'gr+') ),
	(12, '#66d9ef', ('light blue', 'lbl', 'bl+') ),
	(16, '#202020', ('01', 'darkest gray','g1') ),
	(17, '#303030', ('02', 'darker gray','g2') ),
	(18, '#b0b0b0', ('04', 'gray','g4') ),
	(19, '#e0e0e0', ('06', 'lighter gray','g6') ),
	(20, '#d28445', ('09', 'orange','or') ),
	(21, '#fb9fb1', ('pink',)),
	(22, '#725F47', ('brownish',)),
	]

# attribute name : ( foreground, background, focus_foreground, focus_background )
#palette16 = {
...
class ColorMap(object):
	...
	def __init__(self, start_idx=16):
		self.start_idx = start_idx

	def map_labels(self, colors):
		self.colors = colors
		basemap = {}
		for c in colors:
			# iterate labels
			for k in c[2]:
				# map each label to the color's index number
				basemap[k] = c[0] + self.start_idx
		self.labels = basemap
	...
	def color(self, label):
		...
		if label in self.labels:
			label = 'h{0}'.format(self.labels[label])
		if opts:
			label += ',' + opts
		return label

	#def modify_loop(self, loop, colors, palette):
	#	pass

	# start_idx = 0 when we want to modify the 16 default colors
	def modify_terminal_colors(self, loop):
		new_colors = []
		for color in self.colors:
			i = color[0] + self.start_idx
			r,g,b = hex2rgb(color[1])
			new_colors.append((i, r, g, b))
		loop.screen.modify_terminal_palette(new_colors)

	# transform custom palette16 structure to urwid compatible
	def build_palette(self, palette):
		...
		for key in palette:
			fg = palette[key][0]
			bg = palette[key][1]
			fg = self.color(fg) if fg else 'h{0}'.format(self.labels['default_fg'])
			bg = self.color(bg) if bg else 'h{0}'.format(self.labels['default_bg'])
			urwid_palette.append((key, '','','', fg, bg,))

			if len(palette[key]) > 2:
				focus_fg = palette[key][2]
				focus_bg = palette[key][3]
				focus_fg = self.color(focus_fg) if focus_fg else 'h{0}'.format(self.labels['default_fg'])
				focus_bg = self.color(focus_bg) if focus_bg else 'h{0}'.format(self.labels['default_bg'])
				urwid_palette.append((key+'_focus', '','','', focus_fg, focus_bg,))

		return urwid_palette

colormap = ColorMap()

def B16(fg=None, bg=None, w=None, color_map=colormap):
	#focus_fg = None
	#focus_bg = None
	...
	fg = color_map.color(fg) if fg else 'h{0}'.format(color_map.labels['default_fg'])
	bg = color_map.color(bg) if bg else 'h{0}'.format(color_map.labels['default_bg'])
	spec = AttrSpec(fg, bg, colors=256)

	if w is not None:
		#focus_fg = color_map.color(focus_fg)
		#focus_bg = color_map.color(focus_bg)
		#focus = AttrSpec(focus_fg, focus_bg, colors=256)
